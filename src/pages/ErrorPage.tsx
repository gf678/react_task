...
const ErrorPage: React.FC = () => {
  ...
  const status =
    Number.isFinite(rawStatus) && rawStatus > 0
      ? rawStatus
      : 404;
  ...
  const current =
    statusConfig[status] ?? statusConfig[500];
        ...
        <section className="rounded-3xl border border-white/70 bg-white/90 px-6 py-7 shadow-sm backdrop-blur">
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            ...
            <div>
              ...
              >
                {t(`error.${current.label}`)}
              </p>


              <h1 className="mt-2 text-3xl font-bold text-gray-900">
                {t(`error.${current.title}`)}
              </h1>


              <p className="mt-2 text-sm leading-6 text-gray-500">
                {t(`error.${current.description}`)}
              </p>
        ...
        <section className="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm backdrop-blur">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-start gap-4">
              ...
              <div
                className={`flex h-14 w-14 items-center-center justify-center rounded-2xl text-2xl ring-1 ${current.iconBg} ${current.iconRing}`}
              >
                ⚠️
              </div>
              ...
              <div>
                ...
                <h2 className="text-xl font-semibold text-gray-900">
                  {t("error.cannotLoadPage")}
                </h2>


                <p className="mt-2 text-sm leading-6 text-gray-500">
                  {t(`error.${current.helper}`)}
                </p>
